...
def calculate_preliminary_new_lines(file_lines: list[str]) -> int:
...
def create_preliminary_new_lines(markdown_file_path: str, entry: str) -> None:
	...
	if len(journal_lines) >= 2: # if is a new file, we won't need to check for newlines. this isn't checked by the above if statement, as the file could be empty without being generated by the code.
		number_of_preliminary_new_lines = calculate_preliminary_new_lines(journal_lines)
